...
function ProfilePokemonPage() {
  ...
  const formattedId = profilePokemon.id
    ? String(profilePokemon.id).padStart(4, "0")
    : "";
  const formattedName = profilePokemon.name
    ? String(profilePokemon.name).charAt(0).toUpperCase() +
      profilePokemon.name.slice(1).toLowerCase()
    : "";
  const formattedHeight = profilePokemon.height
    ? (profilePokemon.height / 10).toFixed(1)
    : "0.0";
  const formattedWeight = profilePokemon.weight
    ? (profilePokemon.weight / 10).toFixed(1)
    : "0.0";
      ...
      <div className="container mx-auto text-white my-5 grid gap-y-32">
        <div>
          ...
          <div className="flex justify-center mb-5 mt-11">
            <Link to="/">
              <h1 className="text-3xl text-black">Pokédex</h1>
            </Link>
          </div>
          ...
          <div className="grid grid-cols-3 mt-14">
            <div className="grid justify-center">
              <div className="text-center">
                ...
                {profilePokemon.types?.map((type) => {
                  ...
                  return (
                    <p
                      className={`text-lg border py-1 px-40 mb-5 rounded-full ${setButtonColor(
                        type.type.name
                      )}`}
                      key={type.slot}
                    >
                ...
                {profilePokemon.types?.length === 1 && (
                  <div className="py-[19px] px-40 mb-5"></div>
                )}
              </div>
              <div className="text-center">
            ...
            <div className="relative flex justify-center items-center top-16 -z-10">
              <img
                className="absolute animate-spin-custom"
              ...
              <img
                ...
                src={
                  profilePokemon.sprites?.other["official-artwork"]
                    ?.front_default
                }
                alt={profilePokemon.name}
              />
            </div>
            <div className="grid text-xl px-10 mt-20 me-20 mb-10 mr-10">
              ...
              <div className="grid grid-cols-2">
                ...
                <div>
                  ...
                  {profilePokemon.abilities?.map((ability) => {
                    const formattedAbility = ability.ability.name
                      .split("-")
                      .map(
                        (word) =>
                      ...
                      .join(" ");
                    return <p key={ability.ability.name}>{formattedAbility}</p>;
                  })}
                </div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-3">
          <div className="ps-20">
            ...
            <p className="text-xl mt-5">
              Mandi cahaya matahari membuat Ivysaur makin kuat dan menumbuhkan
              kuncup di punggungnya.
            </p>
          </div>
          <div></div>
          <div>
